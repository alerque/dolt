...
@test "stash: stashing on working set with untacked tables only should be nothing to stash" {
    skip_nbf_ld_1
...
@test "stash: stashing untacked tables with --include-untracked flag and popping the table should not be staged" {
    skip_nbf_ld_1
    dolt sql -q "CREATE TABLE new_table (id INT PRIMARY KEY);"
    dolt add .
...
@test "stash: stashing with staged, working set changes with untacked tables should only stash modified working set and staged changes" {
    skip_nbf_ld_1
    dolt sql -q "CREATE TABLE new_table (id INT PRIMARY KEY);"
    dolt add .
